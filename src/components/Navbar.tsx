  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? 'py-3.5 bg-white/95 backdrop-blur-md shadow-sm border-b border-stone-200/80'
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-2 px-5 py-2 rounded-full bg-white/90 border border-stone-200/80 shadow-sm backdrop-blur-xl">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="px-5 py-1.5 text-xs font-bold text-stone-700 hover:text-[#d96b52] hover:bg-stone-100/60 rounded-full transition-all"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            <button
              ...
              className="px-6 py-2.5 text-xs font-bold text-white bg-[#d96b52] hover:bg-[#c85a42] rounded-full transition-all shadow-sm hover:shadow-md flex items-center gap-1.5"
            >
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden mt-3 mx-4 p-6 rounded-3xl bg-white border border-stone-200 shadow-2xl space-y-4">
          <div className="flex flex-col space-y-2">
            {navLinks.map((link) => (
              <a
                ...
                className="px-4 py-2.5 text-sm font-semibold text-stone-800 hover:text-[#d96b52] hover:bg-stone-50 rounded-xl"
              >
                {link.name}
              </a>
            ))}
